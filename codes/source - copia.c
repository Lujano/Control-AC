...
#define CONFIG_TEMP 7 //  Ajuste de temporizaci髇 echa por el usuario.
        ...
        switch(state){
           ...
           case CURRENT_ON:
            ...
            case CURRENT_OFF:
                ...
                output_high(Red_LED); 
                output_high(Green_LED); 
                while(real_time_s() < 10.0){ // Se espera 10 segundos
                    if(state!= RUNNING){
                        state_chg = 1;
                        break;
                    }
           ...
           case CONFIG: // Estado en el que se mide el tiempo pulsado y se cambia el estado del sistema 
                ...
                if (tiempo_activo < 2){
                    if (last_state == CURRENT_ON or last_state== CURRENT_OFF){
             ...
             case CONFIG_TEMP:
                tiempo_activo = medir_pulso(); // Tiempo activo el pulsador, en segundos
                if (tiempo_activo == 0 ) { // Si se dejo de pulsar durante 10 segundos 
                ...
                else if (tiempo_activo < 2 and tiempo_activo >0){
                    time_on = time_on +1;
                    ...
                    if (time_on == 0){
                        time_on = last_time_on; // Por defecto, si no se configura nada
                                              // Se toma la ultima configuraci髇 tomada
                    }
                    state  = CURRENT_ON
...
float medir_pulso(){
    ...
    while (input(Pulsador) == 1){ // esperar el flanco de bajada del pulsador
    }
